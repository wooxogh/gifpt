...
- [ ] **D1** `studio/ai/intent_tracker.py`
  - [ ] `IntentSchema` pydantic model (`entities: list[str]`, `operations: list[str]`)
  - [ ] `extract_intent(user_text) -> IntentSchema` — gpt-4o JSON mode
  - [ ] `@traceable(name="intent_extract", run_type="chain")`
  - [ ] 유닛 테스트 (2–3 fixture)
- [ ] **D2** `check_intent_loss(intent, artifact, stage)` deterministic matcher
  - [ ] `pipeline_capture.py`에 Stage 0 (extract) 삽입
  - [ ] capture dict에 `intent`, `intent_loss: {pseudo_ir, anim_ir, codegen}` 기록
- [ ] **D3** `studio/evaluators/intent_preservation.py` — 5번째 evaluator
  - [ ] `langsmith_adapter.py` 연결
  - [ ] 유닛 테스트
- [ ] **D4 실험 C (observability-only)**: goldset v0 × FULL × gpt-4o × IntentTracker
  - [ ] 5-dim 벡터 수집
  - [ ] 기존 4-dim 대비 intent_preservation이 새 신호를 주는지 확인
- [ ] **D5 중간 판정**: `docs/snapshots/experiment-c-intent-observability.md`
  - [ ] user→pseudo edge에 실제 loss가 잡히는가?
  - [ ] 어느 stage가 최악인가?
  - [ ] 다음 단계 결정: injection mode 구현 / 탐지 refine / 접근 재검토
- [ ] **D6–7 (선택)** injection mode: lost intent를 다음 stage system prompt에 힌트로 주입
  - [ ] 실험 D: baseline vs injection 비교

**DoW:** 5-dim 벡터가 LangSmith에 수집되고, user→pseudo edge loss가 숫자로 드러나거나 "신호 없음"이 확정됨.
