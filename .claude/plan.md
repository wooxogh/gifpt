...
- [ ] `scripts/failure_audit.py` 재실행 + 결과 저장
- [ ] `scripts/weekly_audit.py` 최근 로그 20개 수동 검토
- [ ] 기존 6-stage taxonomy를 **edge 원인**으로 재분류
- [ ] 각 failure class에 edge 매핑 표 작성 (`pseudo→anim`, `anim→codegen`, `codegen→render`, `render→qa`)
- [ ] 실패 20개 각각에 edge 원인 라벨

**산출물 2:** `gifpt/docs/v1-baseline-report.md`

- [ ] `seed_audit.py` 재실행 (16 seed × 전체 stage)
- [ ] 16 케이스별 stage pass/fail 표
- [ ] Failure class 분포 차트 (taxonomy 기준)
- [ ] Edge preservation 증거 4개 수동 카운트:
  - [ ] `_UNKNOWN_HELPERS` 케이스
  - [ ] LSM-Tree (또는 유사 분류 실패) 케이스
  - [ ] `sorting.comparison_shown -2.5` 페널티 케이스
  - [ ] `post_process_manim_code` 21 color + 9 helper regex 증거
- [ ] baseline 4차원 벡터 (pass/cost/latency/judge) 한 장 요약

**산출물 3:** `gifpt/docs/edge-first-measurement.md`

- [ ] Axiom 3개 정리 (delta / edge-first / dynamic graph)
- [ ] Edge 증거 4개 표
- [ ] `scripts/README.md:103-104` 인용문 앵커
- [ ] Delta vs quality 철학 섹션
- [ ] 경쟁 지형 표 (promptfoo / Inspect AI / LangSmith / Weave — build-vs-buy 결과 반영)
- [ ] LangSmith 선택 근거 + custom evaluator 4개 설계

**LangSmith 셋업:**

- [ ] LangSmith 계정 생성 + API key 발급
- [ ] `pip install langsmith` + `LANGCHAIN_TRACING_V2=true` 환경 변수
- [ ] GIFPT 파이프라인에 `@traceable` 데코레이터 주입 (pseudo_ir, anim_ir, codegen, render, qa 5지점)
- [ ] 1 케이스 trace가 LangSmith UI에 나타나는지 확인
- [ ] **Goldset v0 업로드** — `seed_examples.jsonl` (16 case) → LangSmith Dataset

**Week 1 말 Gate 1 체크:**

- [ ] `failure-taxonomy.md` 사용자 승인
- [ ] `v1-baseline-report.md` 사용자 승인
- [ ] `edge-first-measurement.md` 사용자 승인
- [ ] LangSmith goldset v0 업로드 완료
- [ ] GIFPT 파이프라인 trace 1건 이상 LangSmith UI에 표시
- [ ] **면접 Ready 라인 1 확보** — 위 3 문서만으로 narrative 성립 self-check
...
**산출물:** `gifpt/evaluators/` 디렉터리 4 파일

- [ ] `pseudo_anim_preservation.py`
  - [ ] Pseudo IR의 entity/operation set → Anim IR scene list에 보존 여부
  - [ ] Pydantic schema 검증
  - [ ] Pass/fail + 누락 항목 리스트 반환
- [ ] `anim_codegen_preservation.py`
  - [ ] Anim IR scene list → Manim 코드에 `Create()`/`Transform()` 호출 매칭
  - [ ] AST 분석 기반
- [ ] `codegen_render_preservation.py`
  - [ ] Manim 코드 → 렌더 산출물(mp4) 메타데이터
  - [ ] Runtime, scene count, FORBIDDEN AST 검출
- [ ] `render_qa_preservation.py`
  - [ ] 렌더 결과 → Vision QA 점수
  - [ ] `_UNKNOWN_HELPERS` 같은 후처리 개입 건수 카운트
- [ ] 각 evaluator에 단위 테스트 1개씩 (known good/bad case)
- [ ] LangSmith `@evaluator` 등록 + v1 baseline run에서 4 evaluator 모두 실행
- [ ] Edge fail 카운트가 baseline 리포트 숫자와 일치 확인

**DoW:** LangSmith UI에서 16 case × 4 evaluator 매트릭스 확인 가능.
...
**현재 Phase:** Day 1 대기 (Gate 0 완료)
**완료된 Gate:** Gate 0
**다음 마일스톤:** 5개 열린 결정사항 질의 → Week 1 Phase 0 착수
